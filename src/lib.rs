...
fn eval_dt(x: f64, lambda: f64) -> f64 {
    ...
    (3.0 * x * t - 2.0 + 2.0 * lambda.powi(3) * x / y) / z
}

#[pyfunction]
#[pyo3(name = "lambert_solve")]
pub fn py_lambert_solve(
    r1: [f64; 3],
    r2: [f64; 3],
    tof: f64,
    mu: f64,
) -> PyResult<([f64; 3], [f64; 3])> {
    let vec_r1 = Vector3::new(r1[0], r1[1], r1[2]);
    let vec_r2 = Vector3::new(r2[0], r2[1], r2[2]);

    let (v1, v2) = lambert_solver(vec_r1, vec_r2, tof, mu);

    Ok(([v1.x, v1.y, v1.z], [v2.x, v2.y, v2.z]))
}
#[pymodule]
fn rusty_lambert(m: &Bound<'_, PyModule>) -> PyResult<()> {
